...
class JWSVerificatorTest < Test::Unit::TestCase
  ...
  def test_trivial_verification_of_signature
    ...
    payload = {
      ...
      iss:  "government" }
    example jws, payload, { verify_iss: true, iss: "government" }, JSON.generate(payload)
  end
  ...
  def test_trivial_verification_of_issuer
    ...
    e       = assert_raise { example jws, payload, { verify_iss: true, iss: "schumm" }, JSON.generate(payload) }
  ...
  def test_protected_data_is_required
    ...
    payload = { foo: "bar" }
    e       = assert_raise { example jws, payload, {}, JSON.generate(payload) }
